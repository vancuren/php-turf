...
class Measurement {
    ...
    public static function along(LineString $line, float $distance, string $units = 'kilometers')
    {
        $coords = $line->geometry['coordinates'];
        $accumulatedDistance = 0;

        for ($i = 0; $i < count($coords) - 1; $i++) {
            $segmentStart = new Point($coords[$i]);
            $segmentEnd = new Point($coords[$i + 1]);
            $segmentDistance = self::distance($segmentStart, $segmentEnd);

            if ($accumulatedDistance + $segmentDistance > $distance) {
                $overshoot = $distance - $accumulatedDistance;
                return self::destination($segmentStart, $overshoot, self::bearing($segmentStart, $segmentEnd), $units);
            }

            $accumulatedDistance += $segmentDistance;
        }

        return new Point($coords[count($coords) - 1]);
    }
